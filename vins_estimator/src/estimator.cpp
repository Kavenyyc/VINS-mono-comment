...
//对IMU进行处理，传入参数为IMU信息的时间间隔，线加速度，角速度
//IMU预积分
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{   
    //[1]判断是不是第一个imu消息，如果是第一个imu消息，则将当前传入的线加速度和角速度作为初始的加速度和角速度
    if (!first_imu) // first_imu == false：未获取第一帧IMU数据，表示当前上报的IMU数据为第一个IMU数据
    {
        first_imu = true;
        // 将第一帧IMU数据记录下来
...
//[2]创建预积分对象
    ...
    if (frame_count != 0) // 在初始化时，第一帧图像特征点数据没有对应的预积分
    {
        ...
        // [4]更新Rs、Ps、Vs三个向量数组。Rs为旋转向量数组，Ps为位置向量数组，Vs为速度向量数组，数组的大小为WINDOW_SIZE + 1
    ...
    //[5]当frame_count==0的时候表示滑动窗口中还没有图像帧数据，不需要进行预积分，只进行线加速度和角速度初始值的更新
    acc_0 = linear_acceleration;
    gyr_0 = angular_velocity;
}
...
{
    //image的数据类型分别表示feature_id、camera_id、点的x，y，z坐标，在x,y方向上的跟踪速度
    ...
    if (f_manager.addFeatureCheckParallax(frame_count, image, td))//添加特征并检测视差
        marginalization_flag = MARGIN_OLD;//MAGRIN_OLD=0
    else
    ...
    ROS_DEBUG("number of feature: %d", f_manager.getFeatureCount());//计算滑窗中被track过的特征点的数量
    ...
    else // 已经成功初始化，进行正常的VIO紧耦合优化，非线性优化
    {
        TicToc t_solve;
...
bool Estimator::initialStructure()
{
    ...
    map<int, Vector3d> sfm_tracked_points;//用于储存后面SFM重建出的3D点的坐标

    
    // 用于视觉初始化的图像特征点数据
    vector<SFMFeature> sfm_f;
    for (auto &it_per_id : f_manager.feature)//FeatureManager f_manager;//滑窗内的所有点 定义在feature_manager
    {
        ...
        {
            imu_j++; // 表示在一阵图像中特征点的数量，观测到该特征点的图像帧的帧号
    ...
    if(!sfm.construct(frame_count + 1, Q, T, l,
              ...
              sfm_f, sfm_tracked_points))//l 表示上面选出的参考帧的索引 
    {
    ...
    for (int i = 0; frame_it != all_image_frame.end( ); frame_it++)
    {
        ...
        if((frame_it->first) == Headers[i].stamp.toSec()) // all_image_frame与滑动窗口中对应的帧
        {
            ...
            frame_it->second.R = Q[i].toRotationMatrix() * RIC[0].transpose(); // 根据各帧相机坐标系的姿态和外参，得到用各帧IMU坐标系的姿态（对应VINS Mono论文(2018年的期刊版论文)中的公式（6））。//这里的R是针对哪个坐标系啊？
        ...
        for (auto &id_pts : frame_it->second.points) // 对于该帧中的特征点
        {
            ...
            for (auto &i_p : id_pts.second)//frame_it.second数据类型为ImageFrame,points的类型为map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>> > > points;
            // 由于可能有多个相机，所以需要遍历。i_p对应着一个相机所拍图像帧的特征点信息
            {
        ...
        if(pts_3_vector.size() < 6) // 判断pnp所用点对数量是否足够，如果匹配到的3D点数量少于6个，则认为初始化失败
        {
            cout << "pts_3_vector size " << pts_3_vector.size() << endl;
            ROS_DEBUG("Not enough points for solve pnp !");
            return false;
        }
        if (! cv::solvePnP(pts_3_vector, pts_2_vector, K, D, rvec, t, 1)) // pnp求解失败//判断pnp能否求解出旋转向量rvec和平移向量t,D=Mat()，不太明白是啥
        {
        ...
        // pnp求解成功
        cv::Rodrigues(rvec, r);//旋转向量转换成旋转矩阵形式
...
bool Estimator::relativePose(Matrix3d &relative_R, Vector3d &relative_T, int &l)
{
    ...
    //在滑动窗口中，找出和当前帧具有足够对应关系和视差的帧作为参考帧
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
...
void Estimator::setReloFrame(double _frame_stamp, int _frame_index, vector<Vector3d> &_match_points, Vector3d _relo_t, Matrix3d _relo_r)
{
    ...
    for(int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        {
            relo_frame_local_index = i;//记录下滑窗中是哪一帧重定位
            relocalization_info = 1;//时间戳相同，重定位标志置为1
            for (int j = 0; j < SIZE_POSE; j++)//SIZE_POSE=7
                relo_Pose[j] = para_Pose[i][j];//把两个匹配的帧的位置和四元数存储起来
        }
    }
}
