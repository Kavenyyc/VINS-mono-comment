...
class FeatureTracker
{
  public:
    ...
    //prev_pts是上一次发布的帧的图像特征点
    //cur_pts是光流跟踪的前一帧的图像特征点
    //forw_pts是光流跟踪的后一帧的图像特征点
    vector<cv::Point2f> prev_un_pts, cur_un_pts;//归一化相机坐标系下的坐标
    vector<cv::Point2f> pts_velocity;//当前帧相对前一帧特征点沿着x，y方向的像素的移动速度
    vector<int> ids;//能够被跟踪到的特征点的id
    vector<int> track_cnt;//后一帧fow_imd中每个特征点被追踪的次数
    ...
    static int n_id;//用来作为特征点id，每当检测到一个新的特征点，就将n_id作为该特征点的id，然后n_id加1
};
