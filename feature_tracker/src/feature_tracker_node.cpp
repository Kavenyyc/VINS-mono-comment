...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg)
{
    ...
    last_image_time = img_msg->header.stamp.toSec();
    // 3、发布频率控制，并不是美杜如意帧图像，就要进行特征点发布，通过判断间隔时间内的发布次数来进行控制frequency control
    if (round(1.0 * pub_count / (img_msg->header.stamp.toSec() - first_image_time)) <= FREQ)
    {
        PUB_THIS_FRAME = true;
        // reset the frequency control，pub_count=1 时间间隔内的发布频率十分接近设定频率时，更新时间间隔起始时刻，并将数据发布次数置为0
...
{
    //1、初始化，设置句柄handle，设置logger级别
    ros::init(argc, argv, "feature_tracker");
    ros::NodeHandle n("~");
    ros::console::set_logger_level(ROSCONSOLE_DEFAULT_NAME, ros::console::levels::Info);
    //2、从config-->euroc-->euroc_config.yaml文件中读取参数，其中包括话题，图像宽高，追踪的特征点数量等等
    readParameters(n);

    //3、循环读取每个相机的内参
    for (int i = 0; i < NUM_OF_CAM; i++)
        trackerData[i].readIntrinsicParameter(CAM_NAMES[i]);
    //4、如果为鱼眼相机，添加Mask去除图像的边缘噪声
    if(FISHEYE)
    {
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
    ...
    //6、发布feature，实例feature_points，跟踪的特征点，给后端优化使用
    ...
    */
    ros::spin();//ros::spin() 在调用后不会返回，即主程序 mian() 运行到此不往下执行
    return 0;
}
